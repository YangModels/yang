...
if [ -z "$CHECK_BC" ]; then
    echo Must specify version to check against!
    exit 1
fi

#
# Run pyang over all the models in the current directory that also
# exist in the specified version directory, using pyang's
# --check-update-from option.  This requires pyang 1.5 or better, so
# we check this first. Limit check to Cisco native models.
#
